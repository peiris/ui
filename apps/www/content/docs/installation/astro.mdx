...
Run the `bento-kit` init command to setup your project:

```bash
npx bento-kit@latest init
```
...
When running `npx bento-kit@latest init`, your tailwind config for content will be overwritten. To fix this, change the `content` section with the code below to your `tailwind.config.cjs` file:
...
```bash
npx bento-kit@latest add button
```
